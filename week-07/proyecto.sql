...
-- Mínimo revisado: 30 filas en tabla principal y al menos 3 NULL reales.
-- Requisitos cubiertos: NOT NULL, UNIQUE, CHECK, FK, IS NULL y COALESCE.
...
-- Asegurar datos mínimos con NULL reales
...
-- Consulta IS NULL: productos sin nota registrada.
SELECT
    p.id AS "Id",
    p.name AS "Producto sin nota",
    p.notes AS "Nota"
FROM products AS p
WHERE p.notes IS NULL
ORDER BY p.id;

-- Consulta COALESCE: reemplazar NULL por texto visible para el reporte.
SELECT
    p.id AS "Id",
    p.name AS "Producto",
    COALESCE(p.notes, 'Sin observación registrada') AS "Observación"
FROM products AS p
ORDER BY p.id
LIMIT 15;

-- Consulta IS NOT NULL aplicada a teléfono de cliente.
SELECT
