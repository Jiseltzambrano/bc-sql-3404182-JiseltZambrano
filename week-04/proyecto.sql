...
-- Mínimo revisado: 30 filas en tabla principal y 10 en cada secundaria.
-- Requisitos cubiertos: columnas explícitas, alias, WHERE, ORDER BY y paginación.
...
-- Completar datos mínimos de semana 04
...
-- Consulta 1: columnas explícitas y alias descriptivos.
SELECT
    p.sku AS "Código",
    p.name AS "Producto",
...
-- Consulta 2: filtro WHERE por categoría.
SELECT
    p.name AS "Producto de sala",
    p.price AS "Precio"
FROM products AS p
WHERE p.category = 'Sala'
ORDER BY p.price DESC;

-- Consulta 3: filtro WHERE con operador mayor que.
SELECT
    p.name AS "Producto",
    p.stock AS "Unidades disponibles"
FROM products AS p
WHERE p.stock > 5
ORDER BY p.stock DESC;

-- Consulta 4: paginación página 1.
SELECT
    p.id AS "Id",
    p.name AS "Producto"
FROM products AS p
...
-- Consulta 5: paginación página 2.
SELECT
    p.id AS "Id",
    p.name AS "Producto"
FROM products AS p
